...
//printf("%d %d %d %d\n", s->chnnl, s->devid, s->fromval, s->toval);
				if (c)
					...
					value jsonReadChannel(&h, s->chnnl, s->devid);
...
//printf("%d %d %d\n", a->chnnl, a->devid, a->value);
					if (a->chnnl < 8) // channel
					{
						if (c)
						{
							ochannel_set_value(c, (unsigned int)a->chnnl, (unsigned char)a->value);
							ochannel_write(c);
						}
						else
							jsonWriteChannel(&h, a->chnnl, a->devid, (unsigned char)a->value);
					}
					else if (a->chnnl < 10) // bit
					{
						if (c)
							...
							jsonWriteBit(&h, a->chnnl, a->devid, (unsigned char)a->value);
					}
					else if (a->chnnl < 12) // pulse
					{
						if (c)
							...
							jsonWritePulse(&h, a->chnnl, a->devid, a->value);
				}
			}
		}
//printf("\n");
